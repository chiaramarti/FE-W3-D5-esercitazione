...
const onlyInLastMillennium = myArr => myArr.filter(movie => parseInt(movie.Year) >= 1900 && parseInt(movie.Year) <= 2000);
...
const sumAllTheYears = myArr => myArr.reduce((accumulator, currentValue) => accumulator + parseInt(currentValue.Year), 0);
...
const tree = (n) => {
  ...
  let x = '*';
  treeDiv.innerHTML = `<div><span>${x}</span>div>`
  for(let i = 0; i < n; i++) {
    x += '**';
    treeDiv.innerHTML = `<div><span>${x}</span></div>`
  }
